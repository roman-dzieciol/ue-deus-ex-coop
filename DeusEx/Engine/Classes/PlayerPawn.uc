...
exec function ShowClientInventory()
{
    local Inventory Inv;
...
function CoopTravel( string URL )
{
    local PlayerPawn P;

    // Only Host can change levels
    if( Role == ROLE_Authority
    &&  Level.NetMode == NM_ListenServer
    &&  NetConnection(Player) == None )
    {
        xLog( "CoopTravel" #URL );

        if( InStr( Caps(URL), "?PEER" ) == -1 )
            URL = URL $"?peer";

        //if( InStr( Caps(URL), "?LOADONLY" ) == -1 )
        //    URL = URL $"?loadonly";
        ...
        foreach AllActors( class'PlayerPawn', P )
        {
            ...
            {
                xLog( "CoopTravel CLIENT" #(P) );
                P.ClientTravel( URL, TRAVEL_Relative, True );
                //P.Destroy();
            }
        }

        xLog( "CoopTravel HOST" #(self) );
        PreClientTravel();
        ClientTravel( URL, TRAVEL_Relative, True );
    }
}
