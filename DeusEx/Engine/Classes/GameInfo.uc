...
function bool IsRelevant( actor Other )
{
    ...
    // SW: Actors with bSinglePlayer filter don't disappear in Coop
    if
...
function ProcessServerTravel( string URL, bool bItems )
{
    ...
    xLog( "ProcessServerTravel" #URL #bItems );
    ...
    foreach AllActors( class'PlayerPawn', P )
        ...
        {
            xLog( "ProcessServerTravel" #(P) );
            P.ClientTravel( URL, TRAVEL_Relative, bItems );
            P.Destroy();
        }
        else
        {
            xLog( "ProcessServerTravel LOCAL" #(P) );
            LocalPlayer = P;
            P.PreClientTravel();
        }
...
)
{
    ...
    for( PawnLink=Level.PawnList; PawnLink!=None; PawnLink=PawnLink.NextPawn )
    {
        ...
        )
        {
            ...
            ||  (TestPlayer.PlayerReplicationInfo.PlayerName~=InName && TestPlayer.Password~=InPassword) )
            {
                ...
                xLog( "Found matching unoccupied player" #TestPlayer );
    ...
    if( NewPlayer == None )
    {
        xLog( "Couldn't spawn player at " #StartSpot );
...
event AcceptInventory(pawn PlayerPawn)
{
    ...
    //xlog( "All inventory from" @ PlayerPawn.PlayerReplicationInfo.PlayerName @ "is accepted" );
}
...
{
    // SW-- Inventory should spawn copy and die
    return True;
    // --SW
