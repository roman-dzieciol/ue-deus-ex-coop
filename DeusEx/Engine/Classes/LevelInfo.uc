...
var transient PlayerPawn LocalPlayer;
...
{
    xLog( "ServerTravel" #URL #bItems #NextURL #bNextItems #NextSwitchCountdown );
...
final simulated function float GetDistanceFromPlayer( Actor Other )
{
    // Actor.DistanceFromPlayer is not updated on ListenServer ScriptedPawns

    if( LocalPlayer == None )
        LocalPlayer = GetLocalPlayer();

    if( LocalPlayer != None )
        return VSize(LocalPlayer.Location-Other.Location);
