...
function Frob(Actor Frobber, Inventory frobWith)
{
   ...
   // SW: dont recharge if max BE is 0
   if (DeusExPlayer(Frobber) == None || DeusExPlayer(Frobber).EnergyMax == 0 )
      return;


   // SW-- JC uses SP screen, RC use MP mode
   if( Level.NetMode == NM_ListenServer && Viewport(DeusExPlayer(Frobber).Player) != None )
   /*
