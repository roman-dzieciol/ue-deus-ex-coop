...
simulated function float CalculateAccuracy()
{
   ...
   weapskill = GetWeaponSkill();
    ...
    // SW-- Enabled accuracy mods based on health in multiplayer
    /*
   ...
   // SW-- MP weapons dont have forced min accuracy
   /*
   if (Level.NetMode != NM_Standalone)
...
simulated function bool NearWallCheck()
{
    ...
    // SW-- Grenades can be attached to frobbable objects
    /*
...
function Fire(float Value)
{
    ...
    bListenClient = DeusExPlayer(Owner) != None && DeusExPlayer(Owner).PlayerIsListenClient();

    //Log( "Fire" @Owner @AmmoType, name);
    ...
    if ( Level.NetMode != NM_Standalone )  // Turn up the sounds a bit in mulitplayer
    {
        ...
        // SW: ignore if scriptedpawn
        if ( ScriptedPawn(Owner) == None
        && (Owner.IsA('DeusExPlayer') && (DeusExPlayer(Owner).NintendoImmunityTimeLeft > 0.01)
        || (!bClientReady && (!bListenClient))) )
        {
            DeusExPlayer(Owner).bJustFired = False;
            bReadyToFire = True;
            bPointing = False;
    ...
    if (bHandToHand)
    {
        ...
        if (( Level.NetMode != NM_Standalone ) && ScriptedPawn(Owner) == None && !bListenClient )
            bClientReady = False;
    ...
    else if ((ClipCount < ReloadCount) || (ReloadCount == 0))
    {
        if ((ReloadCount == 0) || AmmoType.UseAmmo(1))
        {
            ...
            GotoState('NormalFire');
            if (( Level.NetMode == NM_Standalone )
            || ( DeusExPlayer(Owner) != None && DeusExPlayer(Owner).PlayerIsListenClient()) )
            {
