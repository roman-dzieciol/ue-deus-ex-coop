...
//log("DeusExCarcass::Frob()--------------------------------");
    ...
    if (!bAnimalCarcass && false /* SW: disable picking up carcasses until dropping them works */ )
    {
        // SW-- MP Corpses can be picked up
...
//              log("===>DeusExCarcass:item="$item );
               ...
               // SW-- Corpses let player pick up all ammo and grenades
               /*
...
//log("  bFoundSomething = " $ bFoundSomething);
   ...
   // SW-- MP corpses dont recharge BioEnergy
   /*
   if ((player != None) && (Level.Netmode != NM_Standalone))
   {
   ...
   // SW-- MP corpses dont vanish when frobbed
   /*
...
function AddReceivedItem(DeusExPlayer player, Inventory item, int count)
{
   ...
   DeusExRootWindow(player.rootWindow).hud.receivedItems.AddItem(item, 1);
    ...
    // SW-- MP Corpses dont destroy carried 20mm and WP rocket ammo
    /*
...
auto state Dead
{
    function Timer()
    {
      // SW-- MP corpses stay around indefinitely
        Global.Timer();
      /*
