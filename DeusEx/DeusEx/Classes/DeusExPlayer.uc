...
var travel string ControllerID; // Used to match pawns to players
...
function CreateKeyRing()
{
    if (KeyRing == None)
    {
        ...
        //xlog( "CreateKeyRing" @Level.NetMode @bBeltIsMPInventory );
    }
}
...
function MaintainEnergy(float deltaTime)
{
    ...
    if ((!IsInState('Dying')) && (!IsInState('Paralyzed')))
   {
      ...
      // SW-- BioEnergy doesnt regenerate
      /*
...
*/
        ...
        // SW-- SP weapon skill & speed
        else if ((DeusExWeapon(Weapon) != None) && (Weapon.Mass > 30) && (DeusExWeapon(Weapon).GetWeaponSkill() > -0.25))
        {
            bIsWalking = True;
            newSpeed = defSpeed;
        }
...
exec function ParseRightClick()
{
   ...
   oldFirstItem = Inventory;
    ...
    if (FrobTarget != None)
    {
        ...
        {
            if( PickupNanoKey(NanoKey(FrobTarget)) )
            {
                FrobTarget.Destroy();
                FrobTarget = None;
            }
        ...
        else if (FrobTarget.IsA('Inventory'))
        {
           ...
           // SW-- Put item copy in player inventory
...
exec function ShowMainMenu()
{
    ...
    else
    {
        root = DeusExRootWindow(rootWindow);
        if (root != None)
            root.InvokeMenu(Class'MenuMainRPC');
    }
}
    ...
    )
{
    ...
    if( con == None )
    {
        ...
        //xLog( "GotActiveConversation" @con @invokeActor.name );
    }
    else
    {
        //xLog( "HadActiveConversation" @con @invokeActor.name );
    }
...
exec simulated function newmission( string S )
{
    if (DeusExRootWindow(rootWindow) != None)
        DeusExRootWindow(rootWindow).ClearWindowStack();
    ...
    //ClientTravel(S, TRAVEL_Absolute, False);
}

exec simulated function tra()
{
    newmission("00_Training?Listen");
}

exec simulated function tes()
{
    newmission("test?Listen");
}

exec simulated function OLC( string S )
{
    ConsoleCommand( "obj list class=$S" );
}

exec simulated function EAC( string S )
{
    ConsoleCommand( "editactor class=$S" );
}

exec simulated function EA( string S )
{
    ConsoleCommand( "editactor name=$S" );
}


/*  DefaultProperties Changes -------------------------------------------------
