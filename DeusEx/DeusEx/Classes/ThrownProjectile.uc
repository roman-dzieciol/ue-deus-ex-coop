...
simulated function Tick(float deltaTime)
{
    ...
    if ( Role == ROLE_Authority )
    {
        ...
        // SW-- MP Thrown Projectiles dont disable themselves when owner disappears
        /*
        ...
        if (bProximityTriggered)
        {
            if (bArmed)
            {
                ...
                DistanceFromPlayer = Level.GetDistanceFromPlayer(self);
                ...
                if (proxCheckTime>proxRelevance)
                {
                    ...
                    else
                    {
                            ...
                            /*foreach RadiusActors(class'DeusExPlayer', DP, proxRadius*4)
                            {
                                // only "heavy" pawns will set this off
                                if ((DP != None) && (DP.Mass >= 40))
                                {
                                    // the owner won't set it off, either
                                    if (DP != Owner)
                                    {
                                        dist = DP.Location - Location;
                                        if (VSize(dist) < proxRadius)
                                            if (skillTime == 0)
                                                skillTime = 1.0;
                                    }
